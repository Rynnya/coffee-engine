...
namespace coffee {
    ...
    CommandBuffer::CommandBuffer(const GPUDevicePtr& device, CommandBufferType type) : type { type }, device_ { device }
    {
        switch (type) {
    ...
    CommandBuffer CommandBuffer::createTransfer(const GPUDevicePtr& device)
    {
    ...
    CommandBuffer CommandBuffer::createGraphics(const GPUDevicePtr& device)
    {
        COFFEE_ASSERT(device != nullptr, "Invalid device provided.");
    ...
    CommandBuffer CommandBuffer::createCompute(const GPUDevicePtr& device)
    {
        COFFEE_ASSERT(device != nullptr, "Invalid device provided.");
